...
class SubCategoryController {
    ...
    createSubCategory = async (req, res) => {
        try {
            ...
            const isAllFieldRequired = Helper.allFieldsAreRequired([sub_category, category_id]);
            ...
            if (!isExistCategory) return res.status(STATUS_CODES.NOT_FOUND).json(
                response({
                    type: TYPES.ERROR,
            ...
            if (isExistSubCategory) return res.status(STATUS_CODES.BAD_REQUEST).json(
                response({
                    type: TYPES.ERROR,
            ...
            const data = new SubCategory({
                store_id, category_id, sub_category, image: req?.file?.location ? req?.file?.location : null
            })
